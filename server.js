...
// ---------- Shared "payment confirmed" pipeline ----------
// Marks an order paid and kicks off MooGold auto-fulfill in the background.
// Extracted so every entry point that can confirm a KHQR payment — the
// customer's browser polling handleOrderPaymentStatus, the admin "Verify"
// button (handleAdminVerifyKhqr), and the background sweep
// (pollPendingKhqrOrders) — all go through exactly one fulfill path instead
// of three copies that can drift out of sync.
// `order` must already be the object living inside `data.orders` (same
// reference) so the db.writeDB(data) below persists the paymentStatus flip.
...
// ---------- (B) Background KHQR poller ----------
// The customer's browser only checks payment status while its tab stays
// open (handleOrderPaymentStatus above). If they close it right after
// scanning — a very common pattern — the server never learns the payment
// landed, the order sits at "pending" forever, and MooGold auto-fulfill
// never fires. This sweep runs independently of any browser tab so a real
// payment always eventually gets picked up and delivered.
const KHQR_POLL_INTERVAL_MS = 45 * 1000; // sweep cadence
const KHQR_POLL_ORDER_GAP_MS = 800;      // stagger between orders in one sweep
async function pollPendingKhqrOrders() {
  if (!khqrAutoEnabled()) return; // nothing to check without Bakong creds
  let data;
  try {
    data = db.readDB();
  } catch (e) {
    console.error('[KHQR poll] readDB failed:', e.message);
    return;
  }
  ...
  for (const order of candidates) {
    try {
      ...
      const accountOk = !tx.toAccountId ||
        String(tx.toAccountId).toLowerCase() === KHQR_CONFIG.accountId.toLowerCase();
      if (!amountOk || !currencyOk || !accountOk) {
        console.log('[KHQR poll] mismatch for', order.code, '| amountOk:', amountOk, 'currencyOk:', currencyOk, 'accountOk:', accountOk);
        continue;
      }

      // Re-read right before writing — the customer's own browser poll (or
      // a prior loop iteration) may have already confirmed this order in
      // the gap since we built `candidates`.
      ...
      console.log('[KHQR poll] confirmed payment for', order.code, '(background sweep — tab was likely closed)');
      confirmOrderPaidAndFulfill(freshOrder, freshData);
    } catch (e) {
      console.error('[KHQR poll] check failed for', order.code, ':', e.message);
    }
...
// ---------- (A) Admin manual KHQR verify ----------
// The admin dashboard dropdown (handleUpdateOrderStatus above) only ever
// relabels an order — it never calls fulfillWithMooGold. So an admin
// flipping "pending" → "confirmed" by hand does NOT deliver the Diamonds.
// This endpoint is the correct manual fallback: it re-checks the real
// Bakong transaction for this order's KHQR and, only if a matching payment
// is actually found, runs it through the exact same confirm+fulfill
// pipeline the automatic flows use.
async function handleAdminVerifyKhqr(req, res, params) {
  ...
  if (order.paymentStatus === 'paid') {
    return sendJSON(res, 200, { ok: true, status: 'paid', alreadyPaid: true });
  }
  if (!order.khqr || !order.khqr.md5) {
    return sendJSON(res, 400, { ok: false, error: 'This order has no KHQR attached to verify' });
  }
  if (!khqrAutoEnabled()) {
    return sendJSON(res, 400, { ok: false, error: 'KHQR auto-verify not configured (missing BAKONG_TOKEN/BAKONG_ACCOUNT_ID)' });
  }
  ...
  try {
    ...
    console.log('[Admin] verify-khqr check threw for', order.code, ':', e.message);
    return sendJSON(res, 200, { ok: true, status: 'not_paid', error: e.message });
  }

  if (!result || result.responseCode !== 0 || !result.data) {
    console.log('[Admin] verify-khqr — no matching transaction yet for', order.code);
    return sendJSON(res, 200, { ok: true, status: 'not_paid' });
  }
  ...
  const accountOk = !tx.toAccountId ||
    String(tx.toAccountId).toLowerCase() === KHQR_CONFIG.accountId.toLowerCase();
  if (!amountOk || !currencyOk || !accountOk) {
    console.log('[Admin] verify-khqr mismatch for', order.code, '| amountOk:', amountOk, 'currencyOk:', currencyOk, 'accountOk:', accountOk);
    return sendJSON(res, 200, { ok: true, status: 'mismatch' });
  }

  console.log('[Admin] verify-khqr CONFIRMED for', order.code, 'by admin', session.username);
  confirmOrderPaidAndFulfill(order, data);
  sendJSON(res, 200, { ok: true, status: 'paid' });
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const orderVerifyKhqrMatch = pathname.match(/^\/api\/admin\/orders\/([^/]+)\/verify-khqr$/);
    if (orderVerifyKhqrMatch && method === 'POST') {
      return handleAdminVerifyKhqr(req, res, { orderId: orderVerifyKhqrMatch[1] });
    }
